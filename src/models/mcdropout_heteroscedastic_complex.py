...
class MCDropoutHeteroscedastic(nn.Module):
    def __init__(self, layers, N, M, dropout_rate):
        ...
        self.fc_log_var = nn.Linear(n_in, n_out)

        self.num_epochs = 10

        self.lr = 0.0001
        self.criterion = nn.MSELoss()
        self.optimizer = torch.optim.Adam(reg_list, lr=self.lr)
...
if __name__ == "__main__":
    ...
    model = MCDropoutHeteroscedastic(layers=layers, N=N, M=M, dropout_rate=dropout_rate)

    training_configuration = "mcdropout_heteroscedastic_complex"
