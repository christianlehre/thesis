...
class BayesianRegressor(nn.Module):
    def __init__(self, in_size, out_size, n_batches, dropout_rate):
        ...
        self.bfc_logvar = BayesianLinear(in_size, out_size, self.kl_loss, n_batches, prior_mu=0, prior_sigma=1)
        self.bn = nn.BatchNorm1d(num_features=out_size, track_running_stats=False)
    ...
    def forward(self, x):
        x_ = self.dropout(x)
        y_hat = self.bfc1(x_)
        log_var = self.bn(self.bfc_logvar(x_))
...
if __name__ == "__main__":
    ...
    dataloader = Dataloader(training_set=training_set, validation_set=validation_set, test_set=test_set,
    ...
    model = BayesianRegressor(in_size=input_dim, out_size=output_dim, n_batches=M,
                                           dropout_rate=dropout_rate)

    training_configuration = "sgvb_heteroscedastic_linear"
    ...
    if train:
        ...
        np.savez(path_to_loss, training_loss=training_loss, validation_loss=validation_loss,
